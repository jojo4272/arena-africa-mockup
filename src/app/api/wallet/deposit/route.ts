...
// POST /api/wallet/deposit — AUTH REQUIRED + ATOMIC
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const userId = authedUser.id; // IDOR protection — ignore any userId from body
    ...
    logger.log(LogLevel.INFO, "Deposit via API", { userId, metadata: { amount: amt, provider } });
    return NextResponse.json(result);
  } catch (error: any) {
    logger.log(LogLevel.ERROR, `Deposit failed: ${error.message}`, { userId: authedUser.id });
